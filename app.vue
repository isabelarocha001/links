<template>
    ...
    <main class="container">
      ...
      <template v-else-if="gateReady && gate === 'pass'">
        ...
        <div class="secondary-cards" :class="{ 'secondary-cards--single': !publicChannelEnabled || !telegramBotEnabled || !privsexLinkEnabled }" v-if="configReady && isPt">
          ...
          <button type="button" class="lux-card iq-open-btn" @click="iqStart">
              ...
              <div class="iq-body">
                <template v-if="iqPhase === 'welcome'">
                  ...
                  <p class="iq-intro iq-intro--after-bubble">
                    4 perguntas rápidas.
                    <br />Só quem quer comprar (a partir de R$ 99,90).
                    <br />Se quer só flertar de graça, fecha aí.
                  </p>
                  <button type="button" class="iq-opt iq-opt--primary" @click="iqWelcomeContinue">Beleza, continuar</button>
                </template>
  ...
  <ClientOnly>
    <Teleport to="body">
        ...
        <!-- Popup 2: balão gamificado + Sim amor → PIX R$ 9,90 -->
...
/** Card PrivSex na coluna esquerda quando o link está configurado */
...
function applyServerConfig(data: any) {
  ...
  if (Array.isArray(data.links) && data.links.length) {
    config.links = data.links.filter((l: any) => l && l.label).map((l: any) => {
      ...
      // PrivSex ativo independentemente da antiga data de pausa
...
.secondary-cards .iq-section { margin: 0; padding: 0; min-width: 0; }
.secondary-cards .iq-open-btn {
  flex-direction: column;
...
.secondary-cards .iq-open-ico {
  width: 40px;
  height: 40px;
  border-radius: 10px;
  background: rgba(37, 211, 102, 0.12);
}
.secondary-cards .iq-open-text { flex: 1; }
.secondary-cards .iq-open-arrow { display: none; }
.iq-overlay {
  position: fixed;
