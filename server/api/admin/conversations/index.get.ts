...
export default defineEventHandler(async (event) => {
  ...
  return { ok: true, total: items.length, new_count: items.filter((i: any) => i.is_new).length, unlocked_count: items.filter((i: any) => i.chat_unlocked).length, conversations: items }
})
