...
const listError = ref('')
const filter = ref<'all' | 'new' | 'open' | 'unlocked'>('all')
...
const filteredConversations = computed(() => {
  ...
  if (filter.value === 'unlocked') list = list.filter((c) => c.chat_unlocked)
...
async function loadConversations() {
  ...
  try {
    ...
    unlockedCount.value = res?.unlocked_count || conversations.value.filter((c) => c.chat_unlocked).length
  } catch (e: any) {
    if (e?.statusCode === 401) {
      authed.value = false
      return
        ...
        <section class="ac-inbox">
          <div class="ac-inbox-inner">
            ...
            <div class="ac-tabs">
              ...
              <button type="button" class="ac-tab" :class="{ active: filter === 'unlocked' }" @click="filter = 'unlocked'">
                Chat pago
                <span v-if="unlockedCount" class="ac-tab-count">{{ unlockedCount }}</span>
              </button>
            </div>
              ...
              >
                ...
                <div class="ac-item-body">
                  <div class="ac-item-top">
                    ...
                    <span v-if="c.chat_unlocked" class="ac-unlocked-badge" title="Chat desbloqueado">💚 pago</span>
